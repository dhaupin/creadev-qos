 ...
 */
export async function qosFetch(url, options = {}) {
    const { retries = 3, baseDelayMs = 1000, maxDelayMs = 30000, ms: timeout = 15000, retryStatuses = DEFAULT_RETRY_STATUSES, ...fetchInit } = options;
    const fetchWithOpts = async () => {
        return fetchWithTimeout(url, { ...fetchInit, timeoutMs: timeout });
    };
